"""Quiz Start View - Configurable, timed Multiple-Choice Quiz with live scoring."""
...
class QuizStartView(ctk.CTkFrame):
    def __init__(self, master, db: DatabaseManager, **kwargs):
        ...
        self.current_idx = 0
        self.selected_option_var = ctk.StringVar(value="")
        self.results: List[QuizResult] = []
        
        # Timer tracking
        self.time_limit_sec = 30
        self.remaining_sec = 30
        self.timer_job = None
    ...
    def _build_ui(self):
        ...
        lbl_title = ctk.CTkLabel(
            self.frame_setup,
            text="Quiz Start (Multiple Choice)",
        ...
        lbl_desc = ctk.CTkLabel(
            self.frame_setup,
            text="Customize your quiz parameters, test your knowledge under timed conditions, and review your score breakdown.",
        ...
        self.setup_timer_var = ctk.StringVar(value="30 seconds")
        self.setup_timer_dropdown = ctk.CTkOptionMenu(
            cfg_card,
            variable=self.setup_timer_var,
            values=["No Limit", "15 seconds", "30 seconds", "45 seconds", "60 seconds"],
            height=36,
            fg_color=self.theme_mgr.color("bg_input"),
            button_color=self.theme_mgr.color("accent_primary")
        )
        ...
        # Options Container (Radio / Selection buttons)
        self.options_container = ctk.CTkFrame(self.quiz_card, fg_color="transparent")
        self.options_container.grid(row=2, column=0, sticky="ew", padx=32, pady=(0, 20))
        self.options_container.grid_columnconfigure(0, weight=1)

        self.option_buttons: List[ctk.CTkRadioButton] = []

        # Next button
        self.btn_next_question = ctk.CTkButton(
            self.frame_active,
            text="Submit Answer →",
            command=self.submit_option_and_next,
            height=44,
        ...
        self.res_score_badge = StatBadge(self.frame_results, title="Overall Score", value="100%", icon="🏆")
    ...
    def start_quiz(self):
        ...
        if "15" in t_val:
            ...
            self.time_limit_sec = 0  # No limit

        self.questions = self.db.get_quiz_questions(mode="mcq", category=cat, count=count)
    ...
    def display_current_question(self):
        ...
        # Build options list (from q.options or fallback)
        options = list(q.options)
        if q.answer not in options:
            options.append(q.answer)
        
        # Ensure 4 options by generating distractors if needed
        if len(options) < 4:
            all_other_answers = [other.answer for other in self.questions if other.answer != q.answer]
            for o in all_other_answers:
                if o not in options and len(options) < 4:
                    options.append(o)

        random.shuffle(options)

        # Render option radio buttons
        for idx, opt in enumerate(options):
            ...
            rb.grid(row=idx, column=0, sticky="w", pady=6, padx=12)
    ...
    def _tick_timer(self):
        ...
        if self.remaining_sec <= 0:
            ...
            ToastNotification.show(self, "⏱️ Time's up for this question!", "warning", 2000)
            self.submit_option_and_next()
        else:
    ...
    def submit_option_and_next(self):
        self._stop_timer()
        q = self.questions[self.current_idx]
        user_choice = self.selected_option_var.get().strip()
        is_correct = user_choice.lower() == q.answer.strip().lower()

        self.results.append(QuizResult(
            question_id=q.id or 0,
            prompt=q.prompt,
            user_answer=user_choice if user_choice else "(No answer)",
    ...
    def finish_quiz(self):
        ...
        # Show full-screen summary with score ring
        self._summary_view = QuizSummaryView(
            ...
            accent_color=self.theme_mgr.color("accent_primary"),
        )
